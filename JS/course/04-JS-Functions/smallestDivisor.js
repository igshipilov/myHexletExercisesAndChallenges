// Задача

/*

Реализуйте тело функции smallestDivisor(), используя итеративный процесс.
Функция должна находить наименьший делитель заданного числа. Число, передаваемое в функцию, больше нуля.

Доп. условие: делитель должен быть больше единицы, за исключением случая, когда аргументом является единица
(наименьшим делителем которой является также единица).

Например, наименьший делитель числа 15 это 3.

smallestDivisor(15); // 3
smallestDivisor(17); // 17

Идея алгоритма:

Попробуйте разделить число на 2
Если число делится без остатка, то это наименьший делитель
Если нет, то попробуйте следующий делитель
Если ничего не делит число без остатка, то переданное число является простым,
так что его наименьший делитель —оно само (не считая 1)
Подсказки
Вспомните про оператор % (modulus или остаток от деления).
Он вычисляет остаток от деления одного операнда на другой.

Например, 11 % 5 = 1, а 10 % 2 = 0.
Так что если x % y это 0, то y делит x без остатка

*/

/*

# Алгоритм

Итеративный процесс:
• Базовый случай
• Шаг (counter) // опц., т.к. он нужен не всегда
• Результат в накопителе (acc)

*/

const smallestDivisor = (number) => {
  if (number === 1) {
    return 1;
  }

  const iter = (acc) => {
    if (acc > Math.sqrt(number)) {
      return number;
    }
    if (number % acc === 0) {
      return acc;
    }

    return iter(acc + 1);
  };

  return iter(2);
};

console.log(smallestDivisor(15)); // 3
console.log(smallestDivisor(17)); // 17
