...
// Что, если сначала сделать горизонтальную гистограмму,
// а затем её развернуть?
// По сути, это массив подмассивов, тоже матрица.
// Или это ровно то же самое, что я делал в v02?

// ИДЕЯ
// элементы входного массива используем, чтобы размножить
// соответствующий знаку числа символ:
//  - если элемент массива (число) >0, то `'*'.repeat(el)`
//  - если элемент массива (число) <0, то `'#'.repeat(el)`
...
//   const max = Math.abs(Math.max(...numbers));

//   const bar = numbers[0] > 0 ? '*' : '#';
//   const space = ' ';
//   // const barNeg = '#';
//   // const barPos = '*';

//   const result = numbers.map((number) => bar.repeat(Math.abs(number))
//     .padStart(max, space)
//     .split(''));
...
// ЗАДАЧИ
// Исправить пограничный случай:
// когда на вход поступает массив из только положительных или только отрицательных чисел,
// в функции `getMatrix()` значения `max` и `min` получают `Infinity` и `-Infinity`

const isPositive = (numbers) => numbers[0] > 0;
...
const getMatrix = (numbers, len) => {
  ...
  const arr = isPositive(numbers)
    ? numbersAbs.join('').padEnd(len, '0').split('')
    : numbersAbs.join('').padStart(len, '0').split('');
  // console.log(arr);
